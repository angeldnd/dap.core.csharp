...
namespace angeldnd.dap {
    public abstract partial class Tree<T> {
        public void Filter<T1>(string pattern, Action<T1> callback) where T1 : class, IInTreeElement {
        ...
        public void All<T1>(Action<T1> callback) where T1 : class, IInTreeElement {
            Filter<T1>(PatternMatcherConsts.WildcastSegments, callback);
        }

        public List<T1> Filter<T1>(string pattern) where T1 : class, IInTreeElement {
            List<T1> result = null;
        ...
        public List<T1> All<T1>() where T1 : class, IInTreeElement {
