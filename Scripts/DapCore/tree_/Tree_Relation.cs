...
namespace angeldnd.dap {
    public abstract partial class Tree<T> {
        public T1 GetParent<T1>(string path) where T1 : class, IInTreeElement {
            return Get<T1>(TreeHelper.GetParentPath(Separator, path));
        }

        public void FilterChildren<T1>(string path, Action<T1> callback) where T1 : class, IInTreeElement {
            Filter<T1>(TreeHelper.GetChildrenPattern(Separator, path), callback);
        }

        public List<T1> GetChildren<T1>(string path) where T1 : class, IInTreeElement {
            return Filter<T1>(TreeHelper.GetChildrenPattern(Separator, path));
        }

        public T1 GetAncestor<T1>(string path) where T1 : class, IInTreeElement {
        ...
        public T1 GetDescendant<T1>(string path, string relativePath) where T1 : class, IInTreeElement {
            return Get<T1>(TreeHelper.GetDescendantPath(Separator, path, relativePath));
        }

        public void FilterDescendants<T1>(string path, Action<T1> callback) where T1 : class, IInTreeElement {
            Filter<T1>(TreeHelper.GetDescendantsPattern(Separator, path), callback);
        }

        public List<T1> GetDescendants<T1>(string path) where T1 : class, IInTreeElement {
