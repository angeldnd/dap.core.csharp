...
namespace angeldnd.dap {
    public abstract class TreeInBoth<TO, T> : TreeElement<TO, T>, IInTreeElement, IInTableElement
                                                        where TO : IOwner
                                                        where T : class, IInTreeElement {
        //SILP:IN_BOTH_MIXIN(TreeInBoth)
        protected TreeInBoth(TO owner, string path, Pass pass) : base(owner, pass) {  //__SILP__
            if (owner is ITree) {                                                     //__SILP__
                _Path = path;                                                         //__SILP__
            }                                                                         //__SILP__
        }                                                                             //__SILP__
                                                                                      //__SILP__
        protected TreeInBoth(TO owner, int index, Pass pass) : base(owner, pass) {    //__SILP__
